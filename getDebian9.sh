...
if [ -f "squashfs-root/run/connman/resolv.conf" ]; then
...
./mountunmount.sh "before"
	...
	sudo /usr/sbin/chroot squashfs-root sed -i "s&http://deb.debian.org/debian/&http://archive.debian.org/debian/&g" /etc/apt/sources.list
